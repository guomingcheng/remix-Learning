...
import "./interfaces/IUniswapV2Pair.sol";           //继承，配对合约的接口
import "./UniswapV2ERC20.sol";                      //继承，LP 代币合约
import "./libraries/Math.sol";                      //导入，工具库合约
import "./libraries/UQ112x112.sol";                 //导入，工具库合约
import "./interfaces/IERC20.sol";                   //使用  
import "./interfaces/IUniswapV2Factory.sol";        //使用
import "./interfaces/IUniswapV2Callee.sol";         //使用 
...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    {
        //查询工厂合约的feeTo变量值
        //如果是有值的，就表示要交 0.03 税费
        address feeTo = IUniswapV2Factory(factory).feeTo();
        //如果feeTo不等于0地址,feeOn等于true否则为false
        feeOn = feeTo != address(0);
        //定义k值
        uint256 _kLast = kLast; //上次的 K 值，就是俩个储备量的乘积
        //如果feeOn等于true
        if (feeOn) {
            //如果 k 值不等于 0，那么就表示流动性池子已经创建好了，现在只是在基础上添加流动性
            if (_kLast != 0) {
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        //配对合约持有的 ERC20 代币 - 上次存储 ERC20 代币额度 = 这次添加流动性添加的数额
        //amount0 = 余额0 - 储备0
        ...
        //获取totalSupply,必须在此处定义，因为 totalSupply 可以在mintFee中更新
